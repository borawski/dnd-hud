...
io.on('connection', (socket) => {
    ...
    socket.on('join_encounter', (encounterId) => {
        ...
        if (!encounter) {
            return socket.emit('error', 'encounter not found');
        }

        // If DM, verify ownership
        if (socket.isDM && encounter.dm_user_id !== socket.user.id) {
            return socket.emit('error', 'Not your encounter');
        }

        socket.join(`encounter:${encounterId}`);
        socket.encounterId = encounterId;
        console.log(`${socket.isDM ? 'DM' : 'Player'} joined encounter ${encounterId}`);

        socket.emit('joined_encounter', { encounterId, isDM: socket.isDM });
    });
    ...
    socket.on('update_state', (data) => {
        ...
        if (!socket.encounterId) {
            return socket.emit('error', 'Not in a encounter');
        }

        io.to(`encounter:${socket.encounterId}`).emit('state_update', data);
    });
...
// Get DM's encounters
app.get('/api/dm/encounters', authenticateToken, (req, res) => {
    try {
        const encounters = db.prepare(`
            SELECT * FROM encounters WHERE dm_user_id = ? ORDER BY last_active_at DESC
        `).all(req.user.id);
...
app.post('/api/dm/encounters', authenticateToken, (req, res) => {
    try {
        const { name } = req.body;
        ...
        db.prepare(`
            INSERT INTO encounters (id, name, dm_user_id, last_active_at)
            VALUES (?, ?, ?, ?)
        `).run(encounterId, name, req.user.id, now);

        // Create empty game state for this encounter
        db.prepare(`
            INSERT INTO game_state (encounter_id, active_map, initiative_order, current_turn_index, current_round, combat_started, turn_start_time, log)
            VALUES (?, '', '[]', 0, 1, 0, NULL, '[]')
        `).run(encounterId);
...
// Delete encounter
app.delete('/api/dm/encounters/:encounterId', authenticateToken, (req, res) => {
    try {
        const { encounterId } = req.params;

        // Verify ownership
        const encounter = db.prepare('SELECT * FROM encounters WHERE id = ? AND dm_user_id = ?').get(encounterId, req.user.id);
        if (!encounter) {
            return res.status(403).json({ error: 'Access denied' });
        }

        db.prepare('DELETE FROM encounters WHERE id = ?').run(encounterId);
        db.prepare('DELETE FROM game_state WHERE encounter_id = ?').run(encounterId);
...
// Get encounter game state (PUBLIC - no auth required for player view)
app.get('/api/encounters/:encounterId/gamestate', (req, res) => {
    try {
        const { encounterId } = req.params;
        ...
        const state = db.prepare('SELECT * FROM game_state WHERE encounter_id = ?').get(encounterId);
        if (!state) {
...
// Update encounter game state (DM only - requires authentication)
app.post('/api/encounters/:encounterId/gamestate', authenticateToken, (req, res) => {
    try {
        const { encounterId } = req.params;

        // Verify DM owns this encounter
        const encounter = db.prepare('SELECT * FROM encounters WHERE id = ? AND dm_user_id = ?').get(encounterId, req.user.id);
        if (!encounter) {
            return res.status(403).json({ error: 'Access denied' });
        }
        ...
        // Update encounter last_active_at
        db.prepare('UPDATE encounters SET last_active_at = ? WHERE id = ?').run(new Date().toISOString(), encounterId);

        const newState = db.prepare('SELECT * FROM game_state WHERE encounter_id = ?').get(encounterId);
        ...
        io.to(`encounter:${encounterId}`).emit('state_update', newState);
        res.json(newState);
    } catch (err) {
...
// Import character to specific encounter (DM only)
app.post('/api/encounters/:encounterId/import-character', authenticateToken, async (req, res) => {
    try {
        const { encounterId } = req.params;
